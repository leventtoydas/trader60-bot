...
LIST_IDX  = ["^N225","^IXIC","^GSPC","^FCHI","^RUT","^FTSE","^GDAXI","^SSMI","^DJI"]  # YF’de bazı TR endeksleri yok
...
def _map_to_yf(sym: str) -> str:
    ...
    if s.endswith("USDT") and len(s) > 5:
        # BTCUSDT -> BTC-USD (Yahoo)
        return f"{s[:-4]}-USD"
    if s.upper() == "UKOIL":
        return "BZ=F"
    if s.upper() in {"^BIST100","^XU030"}:
        return ""  # Yahoo desteklemiyor → atla
    return s
...
DEFAULT_SYMBOLS = [s for s in DEFAULT_SYMBOLS if s]  # boşları at
...
def fetch(symbol, tf):
    tfmap = {
        ...
        "30m":("30m","60d"),
        "1h": ("60m","60d"),
        "4h": ("60m","60d"),  # 4H için 60m'den resample yapacağız
    }
